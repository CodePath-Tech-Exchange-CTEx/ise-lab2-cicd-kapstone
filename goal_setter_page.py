...
# goal_setter_page.py
#
# Revamped Goal Setter page with polished UI.
#
#############################################################################
...
def display_goal_card(goal):
        ...
        <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:14px;">
            <div>
                ...
                <div style="display:flex;gap:8px;align-items:center;">
                    <span style="background:{badge_bg};color:{badge_color};border:1px solid {badge_color}30;border-radius:20px;font-size:10px;font-weight:700;padding:2px 10px;text-transform:uppercase;letter-spacing:0.8px;">{goal['type']}</span>
    ...
    with col_minus:
        if st.button("−", key=f"minus_{goal['id']}"):
            ...
            st.rerun()
    with col_val:
        st.markdown(f'<div style="text-align:center;font-family:Barlow Condensed,sans-serif;font-size:20px;font-weight:700;padding-top:4px;">{st.session_state[key_amount]}</div>', unsafe_allow_html=True)
    with col_plus:
